...
class SharpnessScoreIterative(AbstractScore):
    ...
    def __init__(
        ...
        batch_size: int = 32,
        num_workers: int = 4,
        number_of_rand_directions: int = 6,
    ) -> None:
        super().__init__(
        ...
        # This iterative scorer is fixed to six random directions per epoch.
        self.number_of_rand_directions = 6
    ...
    def run(self, config: DictConfig, run_config: DictConfig, run_name: str) -> None:
        ...
        training_run_path = os.path.join(
            self.results_dir,
            self.experiment_id,
            "training",
            run_name,
        ...
        for epoch in epoch_pbar:
            ...
            direction_pbar = tqdm(
                ...
                desc=f"epoch_{epoch} directions",
                leave=False,
                disable=not config.get("progress_bar", False)
            )
            for direction_idx in direction_pbar:
                # Use seed = 1..6 for each epoch checkpoint.
                current_seed = direction_idx + 1

                direction_pbar.set_description(
                    f"epoch_{epoch} [{direction_idx + 1}/{self.number_of_rand_directions}] (seed={current_seed})"
                )
                ...
                # Save per-direction losses and ordering inside epoch_i folder.
                seed_filename = os.path.join(
                    epoch_output_path,
                    f"sharpness_seed_{current_seed}.csv",
                )
                self._save_grid_losses(pt_losses, labels, data, seed_filename)

                seed_scores = self._compute_final_scores(pt_losses, num_samples)
                seed_df = self.create_dataframe(seed_scores, labels, data)
                seed_df.to_csv(
                    os.path.join(epoch_output_path, f"scores_seed_{current_seed}.csv"),
                    index=False,
                )
    ...
    def _load_epoch_checkpoint(self, model: torch.nn.Module, checkpoint_path: str) -> None:
        """Load a model state dict from a specific epoch_i/model.pt checkpoint."""
